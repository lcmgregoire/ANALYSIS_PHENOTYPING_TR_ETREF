...
data_TR_ETREF$TR_G_DAY = data_TR_ETREF$TR_G_INTERVAL / data_TR_ETREF$DURATION

# TR daily / LA on each period = TRrate 

data_TR_ETREF$TR_RATE = data_TR_ETREF$TR_G_DAY / data_TR_ETREF$LA_PERIOD
```
...
transp$TR_RATE = as.numeric(transp$TR_RATE)
...
```{r}
ggplot(transp, aes(x= transp$PERIOD, y=transp$TR_RATE)) + 
  geom_boxplot()
...
ggplot(transp, aes(x= transp$TRENCH, y=transp$TR_RATE)) + 
  geom_boxplot()


ggplot(transp, aes(x= transp$TR_RATE, y=transp$TR_RATE)) + 
  geom_boxplot()


ggplot(transp, aes(x= transp$PERIOD, y=transp$TR_G_DAY)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$BLOCK, y=transp$TR_G_DAY)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$REPETITION, y=transp$TR_G_DAY)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$TRENCH, y=transp$TR_G_DAY)) + 
  geom_boxplot()


ggplot(transp, aes(x= transp$TR_G_DAY, y=transp$TR_G_DAY)) + 
    geom_boxplot()
...
count_before = nrow(transp)
 ...
 data.RmOut$TR_RATE[tf] <- NA
    ...
    out <- boxplot(data.RmOut$TR_G_DAY, plot = FALSE)$out # values to remove
    tf <- which(data.RmOut$TR_G_DAY %in% out) # position in the list
 data.RmOut$TR_G_DAY[tf] <- NA
...
ggplot(data.RmOut, aes(x= data.RmOut$PERIOD, y=data.RmOut$TR_G_DAY)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$BLOCK, y=data.RmOut$TR_G_DAY)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$REPETITION, y=data.RmOut$TR_G_DAY)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$TRENCH, y=data.RmOut$TR_G_DAY)) + 
  geom_boxplot()


ggplot(data.RmOut, aes(x= data.RmOut$TR_G_DAY, y=data.RmOut$TR_G_DAY)) + 
    geom_boxplot()
...
model<-lm(TR_RATE~TRENCH, data = data.RmOut)
aov<-Anova(model)
print(aov)
```
...
```{r plot models TRrate- VPD , eval=FALSE}
rep_colors <- c(
  "1" = "#A65E00",
  "2" = "#1C64A6",
  "3" = "#006747",
...
```{r plot  TRrate- ETREF, eval=FALSE}
...
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = ETREF, y = TR_RATE, color = REPETITION)) +
    ...
    labs(title = paste0("Segmented model - Genotype ", i," - ", geno_name),
      x = "ETREF (mm.day⁻¹)",
    y = "ETr rate (g·h⁻¹·cm⁻²)",      color = "Repetition") +
    theme_bw(base_size = 14)
} else {
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_G_DAY) & !is.na(MEAN_VPD)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_G_DAY ~ 0 + MEAN_VPD, data = cur_data())
...
mutate(
  # Position de base en haut à gauche
  x_base = 0,
  y_top = max(subset$TR_G_DAY) * 1.05,
  y_bottom = y_top - 0.2 * (y_top - min(subset$TR_G_DAY)),
  # répartir verticalement du haut vers le bas
  y_label = y_top - (row_number() - 1) * (y_top - y_bottom) / (n() - 1),
  # petit décalage horizontal pour chaque répétition
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = MEAN_VPD, y = TR_G_DAY, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_G_DAY ~ MEAN_VPD, data = subset_rep)
...
#     # Générer les prédictions
    bp_data <- tryCatch({
      data.frame(MEAN_VPD = seq(min(subset_rep$MEAN_VPD),
                                max(subset_rep$MEAN_VPD),
                                length.out = 100)) %>%
        mutate(TR_G_DAY = predict(bp_model, newdata = .),
               REPETITION = rep)
    }, error = function(e) NULL)
...
n_rep <- nrow(annotations)
if (n_rep > 0) {
 y_base <- max(subset$TR_G_DAY) * 1.05  
y_spacing <- 0.05 * (max(subset$TR_G_DAY) - min(subset$TR_G_DAY))  
...
# # ---------------------------
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = MEAN_VPD, y = TR_G_DAY, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
              aes(x = MEAN_VPD, y = TR_G_DAY, group = REPETITION, color = REPETITION),
              size = 1) +
    geom_text(data = annotations,
              aes(x = x, y = y, label = label, color = REPETITION),
              hjust = 1, vjust = 1, size = 3.5, show.legend = FALSE) +
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_G_DAY) & !is.na(ETREF)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_G_DAY ~ 0 + ETREF, data = cur_data())
  slope <- as.numeric(coef(mod)[["ETREF"]])
  r2_val <- summary(mod)$r.squared
# 
  eq <- paste0("y = ", format(slope, scientific = TRUE, digits = 3), " * x")
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = ETREF, y = TR_G_DAY, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_G_DAY ~ ETREF, data = subset_rep)
    segmented(lm_base, seg.Z = ~ ETREF,
              psi = list(ETREF = median(subset_rep$ETREF, na.rm = TRUE)))
  }, error = function(e) NULL)
...
# # ---------------------------
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = ETREF, y = TR_G_DAY, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
              aes(x = ETREF, y = TR_G_DAY, group = REPETITION, color = REPETITION),
              size = 1) +
    geom_text(data = annotations,
              aes(x = x, y = y, label = label, color = REPETITION),
              hjust = 1, vjust = 1, size = 3.5, show.legend = FALSE) +
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_G_DAY) & !is.na(ETREF_KC)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_G_DAY ~ 0 + ETREF_KC, data = cur_data())
  slope <- as.numeric(coef(mod)[["ETREF_KC"]])
  r2_val <- summary(mod)$r.squared
# 
  eq <- paste0("y = ", format(slope, scientific = TRUE, digits = 3), " * x")
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = ETREF_KC, y = TR_G_DAY, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_G_DAY ~ ETREF_KC, data = subset_rep)
    segmented(lm_base, seg.Z = ~ ETREF_KC,
              psi = list(ETREF_KC = median(subset_rep$ETREF_KC, na.rm = TRUE)))
  }, error = function(e) NULL)
...
if (nrow(all_bp_data) > 0) {
  ...
  p2 <- ggplot(subset, aes(x = ETREF_KC, y = TR_G_DAY, color = REPETITION)) +
  geom_point() +
  geom_line(data = all_bp_data,
            aes(x = ETREF_KC, y = TR_G_DAY, group = REPETITION, color = REPETITION),
            size = 1) +
  geom_text(data = annotations,
            aes(x = x, y = y, label = label, color = REPETITION),
            hjust = 1, vjust = 1, size = 3.5, show.legend = FALSE) +
...
p <- ggplot(df, aes(x = SLOPE)) +
  ...
  labs(
    title = paste ("Histogram of Slope y=ax", trait_name, Yvar, "~", Xvar,  trial ),
    x = "Slope y=ax",
    y = "Density"
  ) +
  theme_minimal()
