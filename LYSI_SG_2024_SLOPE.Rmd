...
transp=read.csv(here::here(opPATH.data,"DATA_TR_ETREF_LYSI_SORGHUM_2024.csv"), dec = ".", sep = ";")
...
for (i in 1:nrow(x = data.RmOut)) {
     data.RmOut$TR_RATE[i][data.RmOut$TR_RATE[i]< 0] <- NA 
      data.RmOut$TR_RATE[i][data.RmOut$REPETITION[i] == 5] <- NA 
     data.RmOut$TR_RATE[i][data.RmOut$REPETITION[i] == 6] <- NA 
    data.RmOut$TR_RATE[i][data.RmOut$N_GENO[i] == "tube vide"] <- NA 
        data.RmOut$TR_RATE[i][data.RmOut$N_GENO[i] == ""] <- NA 
...
```{r}
# sup of 0.0001 outliers ?!
for (i in 1:nrow(x = coef_intercept_metad)) {
        coef_intercept_metad$SLOPE[i][coef_intercept_metad$SLOPE[i]> 0.0001] <- NA 
     }
...
## even if non sign, block and rep should be included as part of the design/ Allow to insure genotypic var is not over or underestimate
...
hr <- H2cal(
  data = as.data.frame(coef_intercept_metad),
  trait = "SLOPE",
  gen.name = "N_GENO",
  rep.n = 4,  # nombre de répétitions si applicable
