---
title: "Analysis Tr-ETref Lysimeter Sorghum 2024"
...
### integrate LA over time

```{r import data TR - ETREF and LEAF AREA}

LA_HARVEST= read.csv2(file = paste0(opPATH.data, "DATA_TE_LYSI_SG_2024.csv"), dec = ".")
 panel <- read.csv(here::here(opPATH.data, "/panel_sorghum.csv"), dec = ".", sep = ";")
data_TR_ETREF= read.csv2(file = paste0(opPATH.data, "DATA_TR_ETREF_LYSI_SG_2024.csv"), dec = ".")


data_TR_ETREF$ID_GENO_TRT =  paste(data_TR_ETREF$N_GENO, data_TR_ETREF$REPETITION, sep = "_")
data_TR_ETREF$ID_PERIOD = paste(data_TR_ETREF$ID_GENO_TRT, data_TR_ETREF$INTERVAL_GROUP, sep = "_")

data_TR_ETREF = split(data_TR_ETREF, data_TR_ETREF$GROUP)
data_TR_A = data_TR_ETREF$A
data_TR_B = data_TR_ETREF$B

LA_HARVEST= read.csv2(file = paste0(opPATH.data, "DATA_TE_LYSI_SG_2024.csv"), dec = ".")
LA_HARVEST = split(LA_HARVEST, LA_HARVEST$GROUP)
LA_HARVEST_A = LA_HARVEST$A
LA_HARVEST_B = LA_HARVEST$B

rm(LA_HARVEST, data_TR_ETREF) 



SOWING = "22-01-2024" ## define sowing date
HARVEST= "22-04-2024" ## define harvest date
...
```{r calculate LEAf area growth group A }

for (i in 1:nrow(LA_HARVEST_A)){
  
  LA_rep = as.data.frame(LA_HARVEST_A[i,])
  ...
  write.table (results, file =  paste(opPATH.results,"LA_LINEAR_GROWTH_GROUP_A",trial,".csv", sep=""), append = TRUE, col.names = F, row.names = F, sep = ";")
  }



LA_LINEAR_GROWTH_GROUP_A = read.csv2(paste(opPATH.results,"LA_LINEAR_GROWTH_GROUP_A",trial,".csv", sep=""), header = F, dec = ".")
colnames(LA_LINEAR_GROWTH_GROUP_A)= c("GENO_NAME", "N_CERAAS","N_GENO", "REPETITION", "SLOPE")

save(LA_LINEAR_GROWTH_GROUP_A, file = paste(opPATH.obj,"LA_LINEAR_GROWTH_GROUP_A.RData", sep = ""))


LA_LINEAR_GROWTH_GROUP_A$EMERG = EMERG
LA_LINEAR_GROWTH_GROUP_A$HARVEST = HARVEST


LA_DAILY_GROUP_A <- LA_LINEAR_GROWTH_GROUP_A %>%
  rowwise() %>%
  mutate(
...
save(LA_DAILY_GROUP_A, file = paste(opPATH.obj,"LA_DAILY_GROUP_A",trial,".RData", sep = ""))
load(file =  paste(opPATH.obj,"LA_DAILY_GROUP_A",trial,".RData", sep = ""))

TS_GROUP_A = unique(date(LA_DAILY_GROUP_A$dates))
TS_GROUP_A = as.POSIXct(TS_GROUP_A, tz= "UTC")
...
target_dates_GROUP_A = unique(data_TR_A$END)
target_dates_GROUP_A <- as.POSIXct(target_dates_GROUP_A, format = "%d/%m/%Y", tz = "UTC")
target_dates_GROUP_A <- as.Date(target_dates_GROUP_A)
target_dates_GROUP_A <- as.character(target_dates_GROUP_A)

head(target_dates_GROUP_A)





LA_DAILY_GROUP_A$dates = as.POSIXct(LA_DAILY_GROUP_A$dates, format = "%Y/%m/%d", tz = "UTC")
LA_DAILY_GROUP_A$dates = sub("UTC", "",LA_DAILY_GROUP_A$dates )
LA_DAILY_GROUP_A$dates <- as.character(LA_DAILY_GROUP_A$dates)

head(LA_DAILY_GROUP_A$dates)

filtered_LA_GROUP_A <- LA_DAILY_GROUP_A %>% filter(dates %in% target_dates_GROUP_A )

filtered_LA_GROUP_A$ID_GENO_TRT = paste(filtered_LA_GROUP_A$N_CERAAS, filtered_LA_GROUP_A$REPETITION, sep = "_")
head(filtered_LA_GROUP_A$ID_GENO_TRT)




filtered_LA_GROUP_A <- filtered_LA_GROUP_A %>%
  mutate(ID_GENO_TRT = filtered_LA_GROUP_A$ID_GENO_TRT) %>%
  relocate(ID_GENO_TRT, .before = 1)
  write.table (results, file = paste(opPATH.results,"filtered_LA_GROUP_A",trial,".csv", sep=""), append = TRUE, col.names = F, row.names = F, sep = ";")


  


PERIOD <- rep(1:length(target_dates_GROUP_A), length.out = length(target_dates_GROUP_A))
df_dates = data.frame(dates = target_dates_GROUP_A, PERIOD = PERIOD)
df_dates = na.omit(df_dates)



filtered_LA_GROUP_A <- merge(filtered_LA_GROUP_A, df_dates, by = "dates", all.x = TRUE)
filtered_LA_GROUP_A$PERIOD = paste(filtered_LA_GROUP_A$PERIOD, "A", sep = "_")

filtered_LA_GROUP_A$ID_PERIOD = paste(filtered_LA_GROUP_A$ID_GENO_TRT, filtered_LA_GROUP_A$PERIOD, sep = "_")
...
data_TR_A =  merge(
  data_TR_A,
  filtered_LA_GROUP_A[, c("ID_PERIOD", "values")],
  by = "ID_PERIOD",
  all.x = TRUE
)

colnames(data_TR_A)[colnames(data_TR_A) == "values"] <- "LA_PERIOD"
colnames(data_TR_A)[colnames(data_TR_A) == "ETREF_MM_DAY"] <- "ETREF"


```


```{r filtered leaf area growth with TR data group B }


for (i in 1:nrow(LA_HARVEST_B)){
  
  LA_rep = as.data.frame(LA_HARVEST_B[i,])
  df <- data.frame(x= c(0,as.numeric(HARVEST- EMERG)), y= c(0,LA_rep$LA_HARVEST))
  
  fit <- lm(y ~ x, data = df)
  slope <- as.numeric(coef(fit)[2])
  results = as.data.frame(t(c(LA_rep$GENO_NAME[1], LA_rep$N_CERAAS[1], LA_rep$N_GENO[1], LA_rep$REPETITION[1], as.numeric(slope))))
  write.table (results, file =  paste(opPATH.results,"LA_LINEAR_GROWTH_GROUP_B",trial,".csv", sep=""), append = TRUE, col.names = F, row.names = F, sep = ";")
  }



LA_LINEAR_GROWTH_GROUP_B = read.csv2(paste(opPATH.results,"LA_LINEAR_GROWTH_GROUP_B",trial,".csv", sep=""), header = F, dec = ".")
colnames(LA_LINEAR_GROWTH_GROUP_B)= c("GENO_NAME", "N_CERAAS","N_GENO", "REPETITION", "SLOPE")

save(LA_LINEAR_GROWTH_GROUP_B, file = paste(opPATH.obj,"LA_LINEAR_GROWTH_GROUP_B.RData", sep = ""))


LA_LINEAR_GROWTH_GROUP_B$EMERG = EMERG
LA_LINEAR_GROWTH_GROUP_B$HARVEST = HARVEST


LA_DAILY_GROUP_B <- LA_LINEAR_GROWTH_GROUP_B %>%
  rowwise() %>%
  mutate(
    dates = list(seq.Date(EMERG, HARVEST, by = "day")),
    values = list(cumsum(rep(SLOPE, length(dates))))
...
save(LA_DAILY_GROUP_B, file = paste(opPATH.obj,"LA_DAILY_GROUP_B",trial,".RData", sep = ""))
load(file =  paste(opPATH.obj,"LA_DAILY_GROUP_B",trial,".RData", sep = ""))

TS_GROUP_B = unique(date(LA_DAILY_GROUP_B$dates))
TS_GROUP_B = as.POSIXct(TS_GROUP_B, tz= "UTC")
...
# filter on based of weight measurement 
target_dates_GROUP_B = unique(data_TR_B$END)
target_dates_GROUP_B <- as.POSIXct(target_dates_GROUP_B, format = "%d/%m/%Y", tz = "UTC")
target_dates_GROUP_B <- as.Date(target_dates_GROUP_B)
target_dates_GROUP_B <- as.character(target_dates_GROUP_B)

head(target_dates_GROUP_B)





LA_DAILY_GROUP_B$dates = as.POSIXct(LA_DAILY_GROUP_B$dates, format = "%Y/%m/%d", tz = "UTC")
LA_DAILY_GROUP_B$dates = sub("UTC", "",LA_DAILY_GROUP_B$dates )
LA_DAILY_GROUP_B$dates <- as.character(LA_DAILY_GROUP_B$dates)

head(LA_DAILY_GROUP_B$dates)

filtered_LA_GROUP_B <- LA_DAILY_GROUP_B %>% filter(dates %in% target_dates_GROUP_B )

filtered_LA_GROUP_B$ID_GENO_TRT = paste(filtered_LA_GROUP_B$N_CERAAS, filtered_LA_GROUP_B$REPETITION, sep = "_")
head(filtered_LA_GROUP_B$ID_GENO_TRT)




filtered_LA_GROUP_B <- filtered_LA_GROUP_B %>%
  mutate(ID_GENO_TRT = filtered_LA_GROUP_B$ID_GENO_TRT) %>%
  relocate(ID_GENO_TRT, .before = 1)
  write.table (results, file = paste(opPATH.results,"filtered_LA_GROUP_B",trial,".csv", sep=""), append = TRUE, col.names = F, row.names = F, sep = ";")


  


PERIOD <- rep(1:length(target_dates_GROUP_B), length.out = length(target_dates_GROUP_B))
df_dates = data.frame(dates = target_dates_GROUP_B, PERIOD = PERIOD)
df_dates = na.omit(df_dates)



filtered_LA_GROUP_B <- merge(filtered_LA_GROUP_B, df_dates, by = "dates", all.x = TRUE)
filtered_LA_GROUP_B$PERIOD = paste(filtered_LA_GROUP_B$PERIOD, "B", sep = "_")

filtered_LA_GROUP_B$ID_PERIOD = paste(filtered_LA_GROUP_B$ID_GENO_TRT, filtered_LA_GROUP_B$PERIOD, sep = "_")
...
data_TR_B =  merge(
  data_TR_B,
  filtered_LA_GROUP_B[, c("ID_PERIOD", "values")],
  by = "ID_PERIOD",
  all.x = TRUE
)

colnames(data_TR_B)[colnames(data_TR_B) == "values"] <- "LA_PERIOD"
colnames(data_TR_B)[colnames(data_TR_B) == "ETREF_MM_DAY"] <- "ETREF"
...
### calcul ETr rate : TR / LA

```{r calcul ETr rate  }
data_TR_ETREF = rbind(data_TR_A, data_TR_B)
data_TR_ETREF$TR_MM_INTERVAL = as.numeric(data_TR_ETREF$TR_MM_INTERVAL)


data_TR_ETREF$TR_MM_DAY = data_TR_ETREF$TR_MM_INTERVAL / data_TR_ETREF$DURATION

# TR daily / LA on each period = TRrate 

data_TR_ETREF$TR_RATE_MM_DAY_CM = data_TR_ETREF$TR_MM_DAY / data_TR_ETREF$LA_PERIOD
```

```{r ETREF KC }

data_TR_ETREF$KC = as.numeric(data_TR_ETREF$KC)
data_TR_ETREF$ETREF_KC = data_TR_ETREF$ETREF * data_TR_ETREF$KC
...
metadata = read.csv2(paste(opPATH.data,"METADATA_LYSI_SG_2024.csv", sep=""), header = T, dec = ".")


# Only keep the LA_PEIROD column and the one you want to merge in TR- ETREF df 
transp =  merge(
  data_TR_ETREF, 
  metadata  [, c("SERIAL_ORDER", "TRENCH", "BLOCK")],
  by = "SERIAL_ORDER",
  all.x = TRUE
)
...
colnames(transp)[colnames(transp) == "INTERVAL"] <- "PERIOD"


transp$N_GENO=as.factor(transp$N_GENO)
transp$REPETITION=as.factor(transp$REPETITION) 

transp$INTERVAL_GROUP=as.factor(transp$INTERVAL_GROUP)
transp$PERIOD=as.factor(transp$PERIOD)

transp$TRENCH = as.factor(transp$TRENCH)
transp$BLOCK = as.factor(transp$BLOCK)
transp$GENO_TRT= paste(transp$N_GENO, transp$REPETITION, sep = "_")
transp$GENO_TRT = as.factor(transp$GENO_TRT)


transp$TR_RATE_MM_DAY_CM = as.numeric(transp$TR_RATE_MM_DAY_CM)
...
nlevels(transp$PERIOD)
nlevels(transp$INTERVAL_GROUP)
...
### visualisation raw Tr data among repetition / block / trench

```{r}
ggplot(transp, aes(x= transp$PERIOD, y=transp$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$BLOCK, y=transp$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$REPETITION, y=transp$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$TRENCH, y=transp$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()


ggplot(transp, aes(x= transp$TR_RATE_MM_DAY_CM, y=transp$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()


ggplot(transp, aes(x= transp$PERIOD, y=transp$TR_MM_DAY)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$BLOCK, y=transp$TR_MM_DAY)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$REPETITION, y=transp$TR_MM_DAY)) + 
  geom_boxplot()

ggplot(transp, aes(x= transp$TRENCH, y=transp$TR_MM_DAY)) + 
  geom_boxplot()


ggplot(transp, aes(x= transp$TR_MM_DAY, y=transp$TR_MM_DAY)) + 
    geom_boxplot()
...
count_before = nrow(transp)
 ...
 data.RmOut <- data.RmOut %>%
  mutate(
    TR_RATE_MM_DAY_CM = ifelse(
      TR_RATE_MM_DAY_CM < 0 |
      REPETITION %in% c(5, 6) |
      N_GENO %in% c("tube vide", ""),
      NA,
      TR_RATE_MM_DAY_CM
    )
  )
 ...
 data.RmOut <- data.RmOut %>%
  mutate(
    ...
    out <- boxplot(data.RmOut$TR_RATE_MM_DAY_CM, plot = FALSE)$out # values to remove
    tf <- which(data.RmOut$TR_RATE_MM_DAY_CM %in% out) # position in the list
 data.RmOut$TR_RATE_MM_DAY_CM[tf] <- NA
    


    out <- boxplot(data.RmOut$TR_MM_DAY, plot = FALSE)$out # values to remove
    tf <- which(data.RmOut$TR_MM_DAY %in% out) # position in the list
 data.RmOut$TR_MM_DAY[tf] <- NA
...
```{r visualisation ETr rate after cleaning }
ggplot(data.RmOut, aes(x= data.RmOut$PERIOD, y=data.RmOut$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$BLOCK, y=data.RmOut$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$REPETITION, y=data.RmOut$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$TRENCH, y=data.RmOut$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()


ggplot(data.RmOut, aes(x= data.RmOut$TR_RATE_MM_DAY_CM, y=data.RmOut$TR_RATE_MM_DAY_CM)) + 
  geom_boxplot()


ggplot(data.RmOut, aes(x= data.RmOut$PERIOD, y=data.RmOut$TR_MM_DAY)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$BLOCK, y=data.RmOut$TR_MM_DAY)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$REPETITION, y=data.RmOut$TR_MM_DAY)) + 
  geom_boxplot()

ggplot(data.RmOut, aes(x= data.RmOut$TRENCH, y=data.RmOut$TR_MM_DAY)) + 
  geom_boxplot()


ggplot(data.RmOut, aes(x= data.RmOut$TR_MM_DAY, y=data.RmOut$TR_MM_DAY)) + 
    geom_boxplot()

```
 not rmd too much for the next step
```{r anova variability among block and rep }

  model<-lm(TR_RATE_MM_DAY_CM~REPETITION, data = data.RmOut)
aov<-Anova(model)
print(aov)


model<-lm(TR_RATE_MM_DAY_CM~BLOCK, data = data.RmOut)
aov<-Anova(model)
print(aov)



model<-lm(TR_RATE_MM_DAY_CM~TRENCH, data = data.RmOut)
aov<-Anova(model)
print(aov)
```
...
## loop to save slope values "Estimate" , AIC R2 and R2 adj 
 for ( i in levels(data$N_GENO) ) {
  ...
  y <- subset_geno_trt$TR_RATE_MM_DAY_CM
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_RATE_MM_DAY_CM) & !is.na(MEAN_VPD)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
  ...
  if (length(valid_reps) > 0) {
    ...
    label_lin <- subset %>%
      group_by(REPETITION) %>%
      reframe({
        mod <- lm(TR_RATE_MM_DAY_CM ~ 0 + MEAN_VPD, data = cur_data())
      ...
      mutate(
        ...
        y_label = seq(max(subset$TR_RATE_MM_DAY_CM) * 1.05,
                      max(subset$TR_RATE_MM_DAY_CM) * 0.85,
                      length.out = n())
      )

    p1 <- ggplot(subset, aes(MEAN_VPD, TR_RATE_MM_DAY_CM, color = REPETITION)) +
      geom_point() +
    ...
    for (rep in unique(subset$REPETITION)) {
      ...
      bp_model <- tryCatch({
        lm_base <- lm(TR_RATE_MM_DAY_CM ~ MEAN_VPD, data = subset_rep)
        segmented(
          lm_base,
          seg.Z = ~ MEAN_VPD,
      ...
      if (!is.null(bp_model)) {
        ...
        bp_pred$TR_RATE_MM_DAY_CM <- predict(bp_model, newdata = bp_pred)
    ...
    annotations <- annotations %>%
      ...
      mutate(
        x = x_limits[2],
        y = seq(max(subset$TR_RATE_MM_DAY_CM) * 1.05,
                max(subset$TR_RATE_MM_DAY_CM) * 0.85,
                length.out = n())
      )

    p2 <- ggplot(subset, aes(MEAN_VPD, TR_RATE_MM_DAY_CM, color = REPETITION)) +
      geom_point() +
      geom_line(data = all_bp_data,
                aes(MEAN_VPD, TR_RATE_MM_DAY_CM, group = REPETITION),
                linewidth = 1) +
      geom_text(
        data = annotations,
      ...
      labs(
        ...
        x = "VPD (kPa)",
        y = "TR rate (mm·day⁻¹·cm⁻²)", color = "Repetition"
      ) +
      theme_bw(base_size = 14) +
      coord_cartesian(xlim = x_limits)
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_RATE_MM_DAY_CM) & !is.na(ETREF)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_RATE_MM_DAY_CM ~ 0 + ETREF, data = cur_data())
...
mutate(
  ...
  x_base = 0,
  y_top = max(subset$TR_RATE_MM_DAY_CM) * 1.05,
  y_bottom = y_top - 0.2 * (y_top - min(subset$TR_RATE_MM_DAY_CM)),
...
p1 <- ggplot(subset, aes(x = ETREF, y = TR_RATE_MM_DAY_CM, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_RATE_MM_DAY_CM ~ ETREF, data = subset_rep)
...
#     # Générer les prédictions
    bp_data <- tryCatch({
      data.frame(ETREF = seq(min(subset_rep$ETREF),
                                ...
                                length.out = 100)) %>%
        mutate(TR_RATE_MM_DAY_CM = predict(bp_model, newdata = .),
               REPETITION = rep)
    }, error = function(e) NULL)
...
if (n_rep > 0) {
 y_base <- max(subset$TR_RATE_MM_DAY_CM) * 1.05  
y_spacing <- 0.05 * (max(subset$TR_RATE_MM_DAY_CM) - min(subset$TR_RATE_MM_DAY_CM))  
...
annotations <- annotations %>%
  ...
  annotations <- data.frame(
    ...
    x = max(subset$ETREF) * 1.05,
    y = max(subset$TR_RATE_MM_DAY_CM),
    stringsAsFactors = FALSE
  )
}
# 
...
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = ETREF, y = TR_RATE_MM_DAY_CM, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
              aes(x = ETREF, y = TR_RATE_MM_DAY_CM, group = REPETITION, color = REPETITION),
              size = 1) +
...
```{r model intercept TRrate - ETREF_KC 0}
...
```{r plot TRrate- ETREF_KC, eval=FALSE}
rep_colors <- c(
  "1" = "#A65E00",
  "2" = "#1C64A6",
  "3" = "#006747",
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_RATE_MM_DAY_CM) & !is.na(ETREF_KC)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_RATE_MM_DAY_CM ~ 0 + ETREF_KC, data = cur_data())
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = ETREF_KC, y = TR_RATE_MM_DAY_CM, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_RATE_MM_DAY_CM ~ ETREF_KC, data = subset_rep)
...
# # ---------------------------
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = ETREF_KC, y = TR_RATE_MM_DAY_CM, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
              aes(x = ETREF_KC, y = TR_RATE_MM_DAY_CM, group = REPETITION, color = REPETITION),
              size = 1) +
    geom_text(data = annotations,
              aes(x = x, y = y, label = label, color = REPETITION),
              hjust = 1, vjust = 1, size = 3.5, show.legend = FALSE) +
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_MM_DAY) & !is.na(MEAN_VPD)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_MM_DAY ~ 0 + MEAN_VPD, data = cur_data())
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = MEAN_VPD, y = TR_MM_DAY, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_MM_DAY ~ MEAN_VPD, data = subset_rep)
...
n_rep <- nrow(annotations)
if (n_rep > 0) {
 y_base <- max(subset$TR_MM_DAY) * 1.05  
y_spacing <- 0.05 * (max(subset$TR_MM_DAY) - min(subset$TR_MM_DAY))  
...
# # ---------------------------
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = MEAN_VPD, y = TR_MM_DAY, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
              aes(x = MEAN_VPD, y = TR_MM_DAY, group = REPETITION, color = REPETITION),
              size = 1) +
    geom_text(data = annotations,
              aes(x = x, y = y, label = label, color = REPETITION),
              hjust = 1, vjust = 1, size = 3.5, show.legend = FALSE) +
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_MM_DAY) & !is.na(ETREF)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_MM_DAY ~ 0 + ETREF, data = cur_data())
  slope <- as.numeric(coef(mod)[["ETREF"]])
  r2_val <- summary(mod)$r.squared
# 
  eq <- paste0("y = ", format(slope, scientific = TRUE, digits = 3), " * x")
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = ETREF, y = TR_MM_DAY, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_MM_DAY ~ ETREF, data = subset_rep)
    segmented(lm_base, seg.Z = ~ ETREF,
              psi = list(ETREF = median(subset_rep$ETREF, na.rm = TRUE)))
  }, error = function(e) NULL)
...
# # ---------------------------
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = ETREF, y = TR_MM_DAY, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
              aes(x = ETREF, y = TR_MM_DAY, group = REPETITION, color = REPETITION),
              size = 1) +
    geom_text(data = annotations,
              aes(x = x, y = y, label = label, color = REPETITION),
              hjust = 1, vjust = 1, size = 3.5, show.legend = FALSE) +
...
```{r plot models TR- ETREF_KC, eval=FALSE }
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = ETREF_KC, y = TR_MM_DAY, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
if (nrow(all_bp_data) > 0) {
  ...
  p2 <- ggplot(subset, aes(x = ETREF_KC, y = TR_MM_DAY, color = REPETITION)) +
  geom_point() +
  geom_line(data = all_bp_data,
...
print(sum_up_reg)
## gloablly, higher R2 with linear regression  
```
...
```{r}
sum_up_XY_TR <- final_results %>%
  filter(Yvar == "TR") %>%
    filter(regression == "linear") %>%
...
metad =  data[, c("TRENCH", "BLOCK", "GENO_TRT")]
...
p <- ggplot(df, aes(x = SLOPE)) +
  ...
  labs(
    title = paste ("Histogram of Slope ", trait_name, Yvar, "~", Xvar,  trial ),
    x = "Linear",
    y = "Density"
  ) +
  theme_minimal()
...
m <- lmer(
  SLOPE ~ 1 + (1 | N_GENO) + (1 | BLOCK) + (1 | REPETITION), # random factors
...
write.table(H2_cullis, file = paste0(opPATH.results,"heritability_cullis_",trait_name,"_", Yvar, "_", Xvar, "_", trial, ".csv"), dec = ".", sep = ";")
```


```{r blups}
blup_geno <- ranef(m)$N_GENO ## save blup
colnames(blup_geno) = "BLUP" 
N_GENO = rownames(blup_geno)
N_GENO= gsub("G", "", N_GENO)
N_GENO= gsub(" ", "", N_GENO)

blup_result <- cbind(N_GENO, blup_geno)

  
panel <- panel %>%
  mutate(N_GENO = as.character(N_GENO))

blup_result = left_join(blup_result, panel, by="N_GENO")

  
  head(blup_result)
save(blup_result, file =  paste0(opPATH.obj,"BLUPS_",trait_name,"_", trial, ".RData"))  
write.table(blup_result, file = paste0(opPATH.results,"BLUPS_",trait_name, "_", trial, ".csv"), dec = ".", sep = ";", row.names = FALSE)
```
...
write.table(SLOPE_mean, file = paste0(opPATH.results,trait_name, "_mean_", Yvar, "_", Xvar, "_", trial, ".csv"), dec = ".", sep = ";", row.names = FALSE)
