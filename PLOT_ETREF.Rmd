...
p1 <- 
  ggplot(data = ETREF_HTP_LTP, aes(x = TRIAL, y = ET_MM_HOUR, color = START_END)) +
...
p2 <-
  ggplot(data = ETREF_LYSI, aes(x = SPECIES, y = ETREF_LYSI$ET_MM_DAY, colour = DURATION_DAY )) +
  geom_point(aes(group = DURATION_DAY)) +
  labs(x = "Crop", y = "ETref (mm/day)", color = "Interval (day)") +
  geom_point(aes(y = ET_MM_DAY), size = 3) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
