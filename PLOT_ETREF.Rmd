...
p1 <- 
  ...
  labs(x= NULL,y = "ETref  (mm.h⁻¹)", color = "Interval (hour)") +
...
p2 <- ggplot(data = ETREF_LYSI, aes(x = SPECIES, y = ETREF_MM_DAY, colour = DURATION)) + geom_point(size = 1,aes(group = DURATION)) + labs(x=NULL,y = "ETref (mm.day⁻¹)", color = "Interval (day)") + theme(axis.text.x = element_text(size = 8, angle = 45, hjust = 1))+ scale_y_continuous(name = "ETref (mm.day⁻¹)", breaks = seq(0, 17, 0.5)) +   theme(text = element_text(size = 8))
