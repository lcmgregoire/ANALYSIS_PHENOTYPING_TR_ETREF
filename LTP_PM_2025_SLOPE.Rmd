...
### calcul TR rate : TR / LA, ETREF * KC

```{r calcul TR rate KC   }
data_TR_ETREF=read.csv(here::here(opPATH.data,"DATA_TR_ETREF_LTP_PM_2025.csv"), dec = ".", sep = ";")
data_TR_ETREF$TR_G_INTERVAL = as.numeric(data_TR_ETREF$TR_G_INTERVAL )
data_TR_ETREF$LEAF_AREA = as.numeric(data_TR_ETREF$LEAF_AREA)
data_TR_ETREF$DURATION_H =as.numeric(data_TR_ETREF$DURATION_H)
data_TR_ETREF$TR_G_H = data_TR_ETREF$TR_G_INTERVAL /(data_TR_ETREF$DURATION_H)


data_TR_ETREF$TR_RATE = data_TR_ETREF$TR_G_H / data_TR_ETREF$LEAF_AREA

data_TR_ETREF$TR_RATE = as.numeric(data_TR_ETREF$TR_RATE )
```

```{r ETREF KC }

colnames(data_TR_ETREF)[colnames(data_TR_ETREF) == "ETREF_MM_H"] <- "ETREF"
data_TR_ETREF$ETREF = as.numeric(data_TR_ETREF$ETREF)

data_TR_ETREF$KC = as.numeric(data_TR_ETREF$KC)
data_TR_ETREF$ETREF_KC = data_TR_ETREF$ETREF * data_TR_ETREF$KC
transp= data_TR_ETREF
```


```{r import panel data, message=FALSE}
...
transp$BLOCK_WEEK=as.factor(transp$BLOCK_WEEK) 
transp$GENO_TRT= paste(transp$N_GENO, transp$REPETITION, sep = "_")
transp$WEEK=as.factor(transp$WEEK)
...
ggplot(transp, aes(x= transp$WEEK, y=transp$TR_RATE)) + 
  geom_boxplot()
...
ggplot(transp, aes(x= transp$BLOCK_WEEK, y=transp$TR_RATE)) + 
  geom_boxplot()
...
mean, max, by loteriod, histo  ... removed outliers if necessary : negative transpiration, LA with NA value
...
# 1. Initial filtering (remove rows)
...
# 2. Remove boxplot outliers on TR_RATE
out <- boxplot(data.RmOut$TR_RATE, plot = FALSE)$out

data.RmOut <- data.RmOut %>%
  filter(!TR_RATE %in% out)

# 3. Drop unused factor levels
data.RmOut <- droplevels(data.RmOut)
...
ggplot(data.RmOut, aes(x = PERIOD, y = TR_RATE)) +
  geom_boxplot()

ggplot(data.RmOut, aes(x = WEEK, y = TR_RATE)) +
  geom_boxplot()

ggplot(data.RmOut, aes(x = BLOCK, y = TR_RATE)) +
  geom_boxplot()

ggplot(data.RmOut, aes(x = REPETITION, y = TR_RATE)) +
  geom_boxplot()

ggplot(data.RmOut, aes(x = BLOCK_WEEK, y = TR_RATE)) +
  geom_boxplot()
...
```{r anova TR among rep/block, eval=FALSE}
...
metad =  transp[, c("WEEK", "POSITION", "ROW", "BLOCK", "BLOCK_WEEK", "GENO_TRT", "ID")]
...
```{r anova among geno }
model_slope <- lmer(SLOPE ~ 1 + (1 | N_GENO) + (1 | BLOCK), data = coef_intercept_metad)
print(model_slope)
...
```{r }
model_slope_reduced <- lmer(SLOPE ~ 1 + (1 | BLOCK), data = coef_intercept_metad)
...
m <- lmer(
  SLOPE ~ 1 + (1 | N_GENO) + (1 | BLOCK), # random factors
...
write.table(blup_result, file = paste0(opPATH.results,"BLUPS_",trait_name,"_", Yvar, "_", Xvar, "_", trial, ".csv"), dec = ".", sep = ";",  row.names = FALSE)
