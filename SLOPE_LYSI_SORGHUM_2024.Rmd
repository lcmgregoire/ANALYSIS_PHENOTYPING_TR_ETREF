...
```{r import data }
data <- read.csv(here::here(opPATH.data, "DATA_TR_ETREF_LYSI_SORGHUM_2024.csv"), dec = ".", sep = ";")

data$N_GENO=as.factor(data$N_GENO)

data$REPETITION=as.factor(data$REPETITION) #why 6 ?? it should be 4 repetition !!
 table(data$REPETITION) #forget about 5-6   102 obs (rep 5) and 17 obs (rep 6) = 7 pots * 17 periods  it is probably empty pot ?! 
...
for (i in 1:nrow(x = data)) {
     ...
     data$TR_RATE[i][data$TR_RATE[i]> 0.1] <- NA 
      data$TR_RATE[i][data$REPETITION[i] == 5] <- NA 

     data$TR_RATE[i][data$REPETITION[i] == 6] <- NA 
    data$TR_RATE[i][data$N_GENO[i] == "tube vide"] <- NA 
...
comparison of AIC
```{r results = 'hide' }
outputfile <- file.path(opPATH.obj, "choose_best_model_AIC_LYSI_SG_2024.txt")
...
outputfile <- file.path(opPATH.obj, "choose_best_model_R2_LYSI_SG_2024.txt")
header <- c("GENO_TRT", "N_GENO", "REPETITION", "Estimate", "R2", "R2adj", "AIC", "best_model")
write.table(t(header), file = outputfile, sep = ",", quote = FALSE, col.names = FALSE, row.names = FALSE)
...
data$GENO_TRT =data$ID_GENO_TRT 
...
p <- ggplot(df, aes(x = SLOPE)) +
  ...
  labs(
    title = "Histogram of Slope y=ax - Lysimeter - SG - 2024",
    x = "Slope y=ax ",
    y = "Density"
  ) +
  theme_minimal()
...
## there are extreme high values on the boxplot, and on raw hist, super low R2 need to be removed
    ...
    for (i in 1:nrow(data)) {
        data$R2[i][data$R2[i]< 0.30] <- NA
    data$SLOPE[i][data$SLOPE[i]> 0.0001] <- NA
    }
...
p <- ggplot(df, aes(x = SLOPE)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mean_SLOPE, sd = sd_SLOPE), color = "darkred", size = 1.2) +
  labs(
    title = "Histogram of SLOPE - Lysimeter - SG - 2024",
    x = "SLOPE y=ax",
    y = "Density"
  ) +
  theme_minimal()
...
min_geno_trt <- data$GENO_TRT[which.min(data$R2)]
max_geno_trt <- data$GENO_TRT[which.max(data$R2)]
min_geno_trt
max_geno_trt
