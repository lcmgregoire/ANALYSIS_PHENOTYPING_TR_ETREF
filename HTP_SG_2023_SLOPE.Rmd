...
# load transpiration rate data on 15 minute interval
# inputfile_TR<-read.csv(paste0(opPATH.data,"TR.mat_updated.csv"), sep = ";", header = TRUE)  #draw raw TR smooth normalized by leaf area  
inputfile_TR_G_INTERVAL<-read.csv(paste0(opPATH.data,"ETr_non_normalized_HTP_SG_2023.csv"), sep = ";", header = TRUE)  #draw raw TR smooth normalized by leaf area  ## units mm/15min
...
transp=  transp[, !duplicated(colnames(transp))]
...
ggplot(transp, aes(x= transp$BLOCK, y=transp$TR_MM_H)) + 
  geom_boxplot() 


ggplot(transp, aes(x= transp$REPETITION, y=transp$TR_MM_H)) + 
  geom_boxplot()


ggplot(transp, aes(x= transp$ROW, y=transp$TR_MM_H)) + 
  geom_boxplot()


ggplot(transp, aes(x= transp$COLUMN, y=transp$TR_MM_H)) + 
  geom_boxplot()
...
ggplot(data.RmOut, aes(x= data.RmOut$BLOCK, y=data.RmOut$TR_MM_H)) + 
  geom_boxplot() 


ggplot(data.RmOut, aes(x= data.RmOut$REPETITION, y=data.RmOut$TR_MM_H)) + 
  geom_boxplot()


ggplot(data.RmOut, aes(x= data.RmOut$ROW, y=data.RmOut$TR_MM_H)) + 
  geom_boxplot()


ggplot(data.RmOut, aes(x= data.RmOut$COLUMN, y=data.RmOut$TR_MM_H)) + 
  geom_boxplot()
...
model<-lm(TR_MM_H~REPETITION, data = data.RmOut)
aov<-Anova(model)
print(aov)


model<-lm(TR_MM_H~BLOCK, data = data.RmOut)
aov<-Anova(model)
print(aov)

model<-lm(TR_MM_H~ROW, data = data.RmOut)
aov<-Anova(model)
print(aov)

model<-lm(TR_MM_H~COLUMN, data = data.RmOut)
aov<-Anova(model)
print(aov)
...
## loop to save slope values "Estimate" , AIC R2 and R2 adj 
 for ( i in levels(data$N_GENO) ) {
  ...
  geno_name= unique(subset_geno_trt$GENO_NAME)
       geno_name = as.character(geno_name)
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_MM_H) & !is.na(MEAN_VPD)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_MM_H ~ 0 + MEAN_VPD, data = cur_data())
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = MEAN_VPD, y = TR_MM_H, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_MM_H ~ MEAN_VPD, data = subset_rep)
...
n_rep <- nrow(annotations)
if (n_rep > 0) {
 y_base <- max(subset$TR_MM_H) * 1.05  
y_spacing <- 0.05 * (max(subset$TR_MM_H) - min(subset$TR_MM_H))  
...
# # ---------------------------
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = MEAN_VPD, y = TR_MM_H, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
              aes(x = MEAN_VPD, y = TR_MM_H, group = REPETITION, color = REPETITION),
              size = 1) +
    geom_text(data = annotations,
              aes(x = x, y = y, label = label, color = REPETITION),
              hjust = 1, vjust = 1, size = 3.5, show.legend = FALSE) +
...
data.RmOut$N_GENO = as.factor(data.RmOut$N_GENO)
data.RmOut$REPETITION <- as.factor(data.RmOut$REPETITION)

data.RmOut <- data.RmOut %>%
  mutate(GENO_NAME = sub(".*_", "", data.RmOut$Variety_ID))
data.RmOut$GENO_NAME = as.factor(data.RmOut$GENO_NAME)
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = ETREF, y = TR_MM_H, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = ETREF, y = TR_MM_H, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
...
for (i in levels(data$N_GENO)) {
  ...
  valid_reps <- subset %>%
    group_by(REPETITION) %>%
    filter(sum(!is.na(TR_MM_H) & !is.na(ETREF_KC)) >= 3) %>%
    pull(REPETITION) %>%
    unique()
...
group_by(REPETITION) %>%
reframe({
  mod <- lm(TR_MM_H ~ 0 + ETREF_KC, data = cur_data())
  slope <- as.numeric(coef(mod)[["ETREF_KC"]])
  r2_val <- summary(mod)$r.squared
# 
  eq <- paste0("y = ", format(slope, scientific = TRUE, digits = 3), " * x")
...
# ### Graphique : points, régression, droite prolongée, labels fixes en haut à gauche
p1 <- ggplot(subset, aes(x = ETREF_KC, y = TR_MM_H, color = REPETITION)) +
geom_point() +
# 
# régression locale
geom_smooth(method = "lm", formula = y ~ 0 + x, se = FALSE, linewidth = 1) +
...
# # ---------------------------
  ...
  bp_model <- tryCatch({
    lm_base <- lm(TR_MM_H ~ ETREF_KC, data = subset_rep)
    segmented(lm_base, seg.Z = ~ ETREF_KC,
              psi = list(ETREF_KC = median(subset_rep$ETREF_KC, na.rm = TRUE)))
  }, error = function(e) NULL)
...
# # ---------------------------
if (nrow(all_bp_data) > 0) {
  p2 <- ggplot(subset, aes(x = ETREF_KC, y = TR_MM_H, color = REPETITION)) +
    geom_point() +
    geom_line(data = all_bp_data,
              aes(x = ETREF_KC, y = TR_MM_H, group = REPETITION, color = REPETITION),
              size = 1) +
    geom_text(data = annotations,
              aes(x = x, y = y, label = label, color = REPETITION),
              hjust = 1, vjust = 1, size = 3.5, show.legend = FALSE) +
...
coef_intercept_metad <- coef_intercept_metad %>%
  group_by(ROW) %>%
  filter(!SLOPE %in% boxplot.stats(SLOPE)$out) %>%
  ungroup()
...
```{r anova among geno }
model_slope <- lmer(SLOPE ~ 1 + (1 | N_GENO) + (1 | ROW), data = coef_intercept_metad)
print(model_slope)
...
```{r }
model_slope_reduced <- lmer(SLOPE ~ 1  + (1 | ROW), data = coef_intercept_metad)
...
panel = read.csv(paste0(opPATH.data,"panel_sorghum_ICRISAT.csv"), sep = ";", header = TRUE) ## careful N_GENO CERAAS different from ICRISAT exp
