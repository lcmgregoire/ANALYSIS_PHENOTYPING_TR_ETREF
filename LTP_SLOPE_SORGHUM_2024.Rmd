...
for (i in 1:nrow(data)) {
    data$TR[i][data$TR[i]< 0] <- NA 
    
    data$TR_RATE[i][data$TR_RATE[i]< 0] <- NA 
    data$TR_RATE[i][data$TR_RATE[i]> 0.0005] <- NA 

    data$TR_RATE[i][data$TR_RATE[i]< 0] <- NA 
    data$TR_RATE[i][data$LEAF_AREA[i]< 50] <- NA 

}


## after chking profile TR-ETREF, removed specific outliers

to_filter <- data.frame(
  g = c(80, 81, 84, 141, 149, 157, 161, 192, 195, 223, 252, 258, 269, 271, 278, 285, 299, 316, 319, 322, 360, 389, 389 ),
  rep = c(1,2,1,2,2,2,2,1,2,4,1,3,4,2,4,4,3,4,2,2,4,4,1)
)
...
# Boucle pour enlever la valeur maximale pour chaque combinaison
for (i in 1:nrow(to_filter)) {
  g <- to_filter$g[i]
  r <- to_filter$rep[i]
  
  subset_rows <- which(data$N_GENO == g & data$REPETITION == r)
  geno_subset = data[subset_rows,]
  

  if (length(subset_rows) > 0) {
    values <- data[["TR_RATE"]][subset_rows]
    
    valid_idx <- which(!is.na(values))
    if (length(valid_idx) == 0) next
    
    # Index relatif vers l'index absolu
    rel_idx <- valid_idx[which.max(values[valid_idx])]
    abs_idx <- subset_rows[rel_idx]
    
    # Stocke la ligne supprimée
    removed_rows <- rbind(removed_rows, data[abs_idx, ])
    
    # Supprime la ligne du data.frame
    data <- data[-abs_idx, ]
...
```{r sorting step2}
#based on previous boxplots :

### there are negative values due to irrigation :  need to be removed
for (i in 1:nrow(data)) {
      data$TR_RATE[i][data$TR_RATE[i]<0] <- NA 
    data$TR_RATE[i][data$TR_RATE[i]> 2e-04] <- NA 
}
...
## ANOVA TRrate among repetitions
...
model_slope <- lmer( SLOPE ~ 1 + (1 | N_GENO) + (1 | WEEK), data = data)
print(model_slope)
```


```{r}
model_slope_reduced <- lmer( SLOPE ~ 1  + (1 | WEEK),  data = data)
...
```{r}
dt <- coef_intercept_metad

# Ajuste le modèle avec l'optimiseur 'bobyqa'
model <- lmer(SLOPE ~ 1 + (1 | N_GENO) + (1 | WEEK),
              data = dt)
