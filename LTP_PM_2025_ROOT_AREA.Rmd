...
hr <- H2cal(data = data,
            ...
            gen.name = "N_GENO",
            rep.n = 4,
            fixed.model = "0 + N_GENO + (1 | REPETITION ) + (1 |  BLOCK_WEEK)",
            random.model = " 1 + (1 | N_GENO) +(1 | REPETITION ) + (1 |  BLOCK_WEEK) ",
            plot_diag = TRUE,
            outliers.rm = TRUE)
...
ROOT_AREA_TOTAL_mean<-as.data.frame(with(data, tapply(data$ROOT_SHOOT, N_GENO, mean)))
ROOT_AREA_TOTAL_mean = cbind(rownames(ROOT_AREA_TOTAL_mean),  ROOT_AREA_TOTAL_mean$`with(data, tapply(data$ROOT_SHOOT, N_GENO, mean))`)
...
ROOT_AREA_TOTAL_mean = merge(ROOT_AREA_TOTAL_mean, panel, by= "N_GENO")
